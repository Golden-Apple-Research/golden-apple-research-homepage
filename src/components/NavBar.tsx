...
export function NavBar({ children }: { children?: any }) {
  ...
  return (
    <header class="site-header overflow-visible">
      <nav class="inset-s-0 top-0 z-20 w-full overflow-visible border-b border-default bg-neutral-primary">
        <div class="max-w-9xl mx-auto flex flex-wrap items-center justify-between overflow-visible p-4">
          <a
            href="/"
            class="flex items-center space-x-3 overflow-visible rtl:space-x-reverse"
          >
            <img
            ...
            ></img>
            <span class="self-center overflow-visible text-xl font-semibold whitespace-nowrap text-heading">
              Golden Apple Research
            </span>
          </a>

          <div class="flex items-center space-x-3 md:order-2 rtl:space-x-reverse">
            <Show when={routeContext().session} fallback={<LoginModal />}>
